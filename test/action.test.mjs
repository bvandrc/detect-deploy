...
const sha256 = (body) => createHash('sha256').update(Buffer.from(body)).digest('hex');
 ...
 */
const startServer = async ({ body, changeAfter = null, nextBody = null, status = 200 } = {}) => {
  let requests = 0;
  let current = body;
  const server = http.createServer((req, res) => {
    ...
    requests += 1;
    if (changeAfter !== null && requests > changeAfter) current = nextBody;
...
const runAction = async ({
  ...
  if (recordedHash !== null) {
    ...
    fs.writeFileSync(path.join(stateDir, 'hash'), `${recordedHash}\n`);
  }
  ...
  const recorded = fs.existsSync(path.join(stateDir, 'hash'))
    ? fs.readFileSync(path.join(stateDir, 'hash'), 'utf8').trim()
    : null;
